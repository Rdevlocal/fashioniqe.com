...
const WeeksFeatured = () => {
  const { data: products, isError, isLoading } = useGetProductTypeQuery({ type: 'fashion', query: 'featured=true' });
    ...
    content = (
      <Swiper {...slider_setting} modules={[Navigation]} className="tp-featured-slider-active swiper-container">
        {products.data.map((item) => {
          ...
          const discountedPrice = discount > 0 ? (Number(price) - (Number(price) * Number(discount)) / 100).toFixed(2) : null;

          return (
            <SwiperSlide key={_id} className="tp-featured-item white-bg p-relative z-index-1">
              ...
              <div className="tp-featured-content">
                ...
                <div className="tp-featured-price-wrapper">
                  {discountedPrice ? (
                    <>
                      ...
                      <span className="tp-featured-price old-price">${discountedPrice}</span>
                    </>
                  ) : (
                    <span className="tp-featured-price new-price">${price}</span>
                  )}
                ...
                <div className="tp-product-rating-icon tp-product-rating-icon-2">
                  <Rating allowFraction size={16} initialValue={averageRating} readonly={true} />
                </div>
                <div className="tp-featured-btn">
                  <Link href={`/product-details/${_id}`} className="tp-btn tp-btn-border tp-btn-border-sm">
